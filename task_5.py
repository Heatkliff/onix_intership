from datetime import datetime, timedelta  # Импортировать библиотеку datetime
from random import randrange, sample, randint, choice  # Импортировать библиотеку random
from string import printable as list_chars
...
def use_exception(exception):
    raise eval(exception)("Искуственно вызванное исключение")  # Искусственно создать exception в функции
...
if __name__ == '__main__':
    ...
    # Задачи с пятой недели
    today = datetime.today()  # Определить текущую дату и время
    tomorrow = today + timedelta(days=1)  # добавить к ней 1 день
    tomorrow_str = tomorrow.strftime("%d-%m-%Y %H:%S")  # Конвертировать ее в строку в формате (ДД-ММ-ГГГГ ЧЧ:СС)
    print(tomorrow_str)  # вывести ее в консоль
    date_str = "2020-02-03 09:18:36.000"  # Создать строчную переменную с датой ‘2020-02-03 09:18:36.000’
    date_from_str = datetime.strptime(date_str, "%Y-%d-%m %H:%M:%S.%f")  # конвертировать ее в формат datetime
    # Вывести в консоль отдельно
    print(date_from_str.strftime("%d"))  # день(2 символа)
    print(date_from_str.strftime("%m"))  # месяц(2 символа)
    print(date_from_str.strftime("%Y"))  # год(4 символа)
    print(date_from_str.strftime("%H"))  # часы(2 символа)
    print(date_from_str.strftime("%S"))  # секунды(2 символа)
    # Найти 3 случайных числа между 100 и 999, которые будут делиться на 5
    three_nums_eq_5 = [randrange(100, 999, 5) for i in range(3)]
    print(three_nums_eq_5)  # вывести их в консоль
    # Сгенерировать строку из рандомных символов, длиной в 10 символов
    str_ten_num = ''.join(choice(list_chars) for i in range(10))
    # Сгенерировать 100 рандомных лотерейных билетов
    lottery_tickets = [randint(1000000000, 9999999999) for i in range(100)]  # (10-ти значные числа)
    lottery_tickets_win = sample(lottery_tickets, 2)  # из них рандомно выбрать 2 выигрышных
    print(lottery_tickets_win)  # Вывести выигрышные билеты в консоль
    # потренироваться в использовании конструкции try/except/finally
    try:
        # use_exception("AttributeError")
        use_exception("RuntimeError")
    except RuntimeError:
        print("RuntimeError")
    except AttributeError:
        print("AttributeError")
    finally:
        print("Отработка exception успешна")
